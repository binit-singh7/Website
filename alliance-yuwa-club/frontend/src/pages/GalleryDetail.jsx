...
// Prefix used by the backend to mark activity-sourced virtual albums.
...
function GalleryDetail() {
  ...
  // Determine whether this is an activity-sourced album or a manual gallery album.
  const isActivity = slug.startsWith(ACTIVITY_SLUG_PREFIX)
  // Strip the prefix when calling the activity-images API.
  const activitySlug = isActivity ? slug.slice(ACTIVITY_SLUG_PREFIX.length) : null

  useEffect(() => {
    let isCurrent = true

    const fetcher = isActivity
      ? fetchActivityGalleryAlbum(activitySlug)
  ...
  }, [slug, isActivity, activitySlug])

  const navigate = useCallback(
    (direction) => {
      setSelectedPhoto((current) => ({
        slug,
        index: (current.index + direction + album.images.length) % album.images.length,
      }))
    },
    [album, slug]
  )
  ...
  return (
    <article className="gallery-detail">
      <Seo
        ...
        description={
          album.description ||
          `Browse published photographs from the ${album.title} album by Alliance Yuwa Club in Biratnagar.`
        }
        path={`/gallery/${slug}`}
        type="gallery"
      ...
      />
      <motion.header
        className="page-container gallery-detail__hero"
        initial={{ opacity: 0, y: shouldReduceMotion ? 0 : 18 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: shouldReduceMotion ? 0 : 0.4 }}
      >
        <Link className="text-link" to="/gallery">← Back to gallery</Link>
        <p className="content-eyebrow">{isActivity ? 'Activity album' : 'Photo album'}</p>
        <h1>{album.title}</h1>
        ...
        {isActivity && (
          <Link className="text-link" to={`/activities/${activitySlug}`}>
            View full activity record <span aria-hidden="true">→</span>
          </Link>
        )}
      </motion.header>

      <section
        className="page-container gallery-detail__photos"
      ...
      >
        <div className="gallery-detail__heading">
          <div>
            ...
            <h2 id="album-photos-title">Browse the album</h2>
          </div>
        ...
        {album.images.length ? (
            ...
            {album.images.map((image, index) => {
              ...
              return (
                ...
                >
                  <button
                    type="button"
                    onClick={() => setSelectedPhoto({ slug, index })}
          ...
          <div className="content-message">
            Photographs for this published album will appear here when they are added.
          </div>
        )}
      </section>
